...
class XFlashExt(metaclass=LogBase):
    ...
    def seccfg(self, lockflag):
        ...
        data, guid_gpt = self.xflash.partition.get_gpt(self.mtk.config.gpt_settings, "lu2")
        seccfg_data = None
        partition = None
        if guid_gpt is None:
        ...
        for rpartition in guid_gpt.partentries:
            if rpartition.name == "seccfg":
                ...
                seccfg_data = self.xflash.readflash(
                    ...
                    filename="", parttype="lu2", display=False)
                break
